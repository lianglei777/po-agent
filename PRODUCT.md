...
5. **Ant Design by default.** Use Ant components, tokens, typography, feedback, and interaction patterns for standard UI, with Lucide as the single product icon set. The dark-only workspace uses `#1668dc` for primary actions and `#4096ff` for focus and active emphasis; custom UI is reserved for real Po Agent domain behavior.
...
10. **Generation control is progressive.** The Canvas Agent prepares complete, reviewable nodes without creating paid runs. Users can inspect or adjust Route-defined values, then explicitly start generation from a node or workflow.
