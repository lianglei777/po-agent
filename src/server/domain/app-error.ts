export type AppErrorCode =
  | "VALIDATION_ERROR"
  | "AUTH_REQUIRED"
  | "PASSWORD_CHANGE_REQUIRED"
  | "INVALID_PASSWORD"
  | "AUTH_RATE_LIMITED"
  | "ACCESS_CONTROL_DEVELOPMENT_BYPASS"
  | "SESSION_NOT_FOUND"
  | "FILE_NOT_FOUND"
  | "NOT_A_FILE"
  | "NOT_A_DIRECTORY"
  | "FILE_TOO_LARGE"
  | "MODEL_NOT_FOUND"
  | "UNSUPPORTED_COMMAND"
  | "COMPACTION_NOT_AVAILABLE"
  | "OAUTH_PROVIDER_NOT_FOUND"
  | "PENDING_INPUT_NOT_FOUND"
  | "SKILL_NOT_FOUND"
  | "SKILL_CONFLICT"
  | "SKILL_SEARCH_FAILED"
  | "SKILL_INSTALL_BUSY"
  | "SKILL_INSTALL_FAILED"
  | "SKILL_REMOVE_BUSY"
  | "SKILL_REMOVE_FAILED"
  | "SKILL_CREATE_FAILED"
  | "SKILL_PACK_NOT_FOUND"
  | "SKILL_PACK_INSTALL_BUSY"
  | "SKILL_PACK_INSTALL_FAILED"
  | "SKILL_PACK_UPDATE_FAILED"
  | "SKILL_PACK_REPAIR_FAILED"
  | "SKILL_PACK_REMOVE_FAILED"
  | "SKILL_PACK_BROKEN"
  | "INSTRUCTION_TOO_LARGE"
  | "INSTRUCTION_CONFLICT"
  | "PROJECT_NOT_REGISTERED"
  | "INSTRUCTION_READ_FAILED"
  | "INSTRUCTION_WRITE_FAILED"
  | "INSTRUCTION_DELETE_FAILED"
  | "AGENT_BUSY"
  | "AGENT_GENERATION_DISABLED"
  | "PIPELINE_AGENT_ACTION_NOT_ALLOWED"
  | "PIPELINE_AGENT_PLAN_NOT_FOUND"
  | "PIPELINE_AGENT_PLAN_NOT_EDITABLE"
  | "PIPELINE_AGENT_ACTION_NOT_UNDOABLE"
  | "PIPELINE_AGENT_MODEL_VISION_REQUIRED"
  | "PIPELINE_MEDIA_PREPROCESSOR_UNAVAILABLE"
  | "PIPELINE_MEDIA_ANALYSIS_FAILED"
  | "INSTRUCTION_RELOAD_FAILED"
  | "GENERATION_OPERATION_UNSUPPORTED"
  | "GENERATION_ROUTE_UNAVAILABLE"
  | "GENERATION_ROUTE_NOT_FOUND"
  | "GENERATION_PROVIDER_DISABLED"
  | "GENERATION_USER_AUTHORIZATION_REQUIRED"
  | "GENERATION_IDEMPOTENCY_CONFLICT"
  | "GENERATION_RUN_NOT_FOUND"
  | "GENERATION_RUN_NOT_CONFIRMABLE"
  | "GENERATION_RUN_ACTIVE"
  | "GENERATION_PROVIDER_NOT_FOUND"
  | "GENERATION_CREDENTIAL_NOT_FOUND"
  | "GENERATION_CREDENTIAL_NOT_SUPPORTED"
  | "GENERATION_UPLOAD_FAILED"
  | "GENERATION_PROVIDER_PROTOCOL_ERROR"
  | "GENERATION_PROVIDER_ERROR"
  | "GENERATION_PROVIDER_RATE_LIMITED"
  | "GENERATION_DOWNLOAD_FAILED"
  | "GENERATION_DOWNLOAD_RECOVERY_UNAVAILABLE"
 | "GENERATION_DOWNLOAD_TOO_LARGE"
 | "GENERATION_DOWNLOAD_URL_REJECTED"
  | "PIPELINE_PROJECT_NOT_FOUND"
  | "PIPELINE_ASSET_NOT_FOUND"
  | "PIPELINE_FRAME_NOT_FOUND"
  | "PIPELINE_STAGE_GATED"
  | "PIPELINE_ASSET_LOCKED"
  | "PIPELINE_LLM_FAILED"
  | "PIPELINE_CANVAS_NODE_NOT_FOUND"
  | "PIPELINE_CANVAS_REVISION_CONFLICT"
  | "PIPELINE_WORKFLOW_RUN_ACTIVE"
  | "PIPELINE_WORKFLOW_RUN_NOT_FOUND"
  | "PIPELINE_WORKFLOW_RUN_NOT_CANCELLABLE"
  | "PIPELINE_WORKFLOW_RUN_NOT_RETRYABLE"
  | "PIPELINE_WORKFLOW_RUN_BUSY"
 | "INTERNAL_ERROR";

export class AppError extends Error {
  constructor(
    public readonly code: AppErrorCode,
    message: string,
    public readonly status: number,
    public readonly details?: unknown,
  ) {
    super(message);
    this.name = "AppError";
  }
}
