...
export class QianwenUploadClient{
  ...
  async upload(asset:ProviderInputAsset,credential:string,vendorModel:string):Promise<PreparedGenerationAsset>{
    ...
    if(!response.ok)throw providerError(value,`Qianwen upload policy failed (${response.status})`);
    ...
    if(!uploaded.ok)throw new AppError("GENERATION_UPLOAD_FAILED",`Qianwen OSS upload failed (${uploaded.status})`,502);
...
function rejected(){return new AppError("GENERATION_UPLOAD_FAILED","Qianwen upload URL is not allowed",502)}
...
function providerError(value:unknown,fallback:string){return new AppError("GENERATION_PROVIDER_ERROR",str(obj(value).message)??fallback,502)}
