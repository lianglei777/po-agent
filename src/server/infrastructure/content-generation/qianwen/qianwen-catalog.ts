...
export function createQianwenRoutes(now=new Date().toISOString()):GenerationRoute[]{return [...defs.map(d=>({id:d.id,name:d.name,navigationLabel:d.capability,description:d.description,tags:d.tags,product:"Wan 3.0",capability:d.capability,providerId:QIANWEN_PROVIDER_ID,providerOperation:d.operation,enabled:false,isDefault:false,revision:3,defaults:Object.fromEntries((d.schema.parameters??[]).filter(f=>f.defaultValue!==undefined).map(f=>[f.key,f.defaultValue as JsonValue])),inputSchema:d.schema,adapterConfig:{protocol:"dashscope-media-v1",operation:d.operation,endpointId:"video-synthesis",vendorModel:"wan3.0-video",requestProfile:"wan-3-video-v1",resultProfile:"video-url-v1",assetBindings:d.bindings,pollIntervalMs:15_000,submitMode:"async-task"} as unknown as JsonValue,credentialRef:QIANWEN_CREDENTIAL_REF,createdAt:now,updatedAt:now})),...imageRoutes(now),...extendedVideoRoutes(now)].map(route=>({...route,inputSchema:withGenerationParameterPresentation(route.inputSchema)}));}
function imageRoutes(now:string):GenerationRoute[]{const imageDefs=[{id:"qianwen-z-image-text-to-image",name:"Z-Image Turbo 文生图片",description:"千问 Z-Image Turbo 同步文生图，适合快速生成单张 PNG 图片。",tags:["同步生成","单张PNG","最高2048","提示词改写"],product:"Z-Image Turbo",operation:QIANWEN_Z_IMAGE_TEXT_TO_IMAGE_OPERATION,maxLength:800,model:"z-image-turbo",endpoint:"multimodal-generation",mode:"sync",size:"1024*1536",sizes:["1024*1024","832*1248","1248*832","864*1152","1152*864","720*1280","1280*720","1024*1536","1536*1024"]}] as const;return imageDefs.map(d=>{const fields:GenerationParameterField[]=[select("size","图片尺寸",[...d.sizes],d.size),bool("promptExtend","提示词智能改写",false),{key:"seed",label:"随机种子",type:"number",min:0,max:2_147_483_647}];return{id:d.id,name:d.name,navigationLabel:"text-to-image",description:d.description,tags:[...d.tags],product:d.product,capability:"text-to-image",providerId:QIANWEN_PROVIDER_ID,providerOperation:d.operation,enabled:false,isDefault:false,revision:2,defaults:Object.fromEntries(fields.filter(f=>f.defaultValue!==undefined).map(f=>[f.key,f.defaultValue as JsonValue])),inputSchema:{prompt:{required:true,maxLength:d.maxLength},parameters:fields},adapterConfig:{protocol:"dashscope-media-v1",operation:d.operation,endpointId:d.endpoint,vendorModel:d.model,requestProfile:"messages-text-image-v1",resultProfile:"choices-content-image-v1",assetBindings:[],submitMode:d.mode},credentialRef:QIANWEN_CREDENTIAL_REF,createdAt:now,updatedAt:now} as GenerationRoute;});}
...
function extendedVideoRoutes(now: string): GenerationRoute[] {
  ...
  const definitions: ExtendedVideoDefinition[] = [
    ...
    {id:"qianwen-happyhorse-1-1-reference-to-video",operation:QIANWEN_HAPPYHORSE_REFERENCE_TO_VIDEO_OPERATION,name:"HappyHorse 1.1 参考生视频",description:"HappyHorse 1.1 参考生视频，支持按顺序引用 1–9 张角色或场景图片。",product:"HappyHorse 1.1",capability:"multimodal-to-video",navigationLabel:"reference-to-video",model:"happyhorse-1.1-r2v",profile:"happyhorse-video-v1",schema:{prompt:{required:true,maxLength:5000},parameters:horseParameters(),assets:[image("imageUrls","参考图片",9,true)]},bindings:[{slot:"imageUrls",mediaType:"reference_image",cardinality:"list"}]},
