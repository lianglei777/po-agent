...
正常生成期间由 `generate_image` 或 `generate_video` 在单次工具执行内等待 SQLite Run，Worker 独立轮询供应商。Pi `tool_execution_update` 通过既有 Agent SSE 转发到 Chat，前端按稳定 `toolCallId` 原地更新标准化阶段；模型不负责定时轮询。执行前确认属于回合结束后的异步工作，Chat 通过 Generation Run API 刷新其共享展示状态。`get_generation` 仅用于用户明确查询或中断恢复。工具结果使用本地 `runId` 标识 Po Agent 的持久化 Run，并把最新 Provider Job 的 `providerId` 与 `remoteTaskId` 标准化为 `providerTaskId` 返回；Chat 分别标注本地 Run ID 与供应商 Task ID。下载产物由 application 根据最终 Prompt 生成简短名称提示，filesystem adapter 负责过滤非法字符和 Windows 保留名，文件仍隔离在对应 Run 目录。
