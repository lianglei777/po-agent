...
Chat Composer 开启内容生成后仍提交标准 Agent Prompt，不在浏览器侧先调用 planner 并旁路创建 `direct-ui` Run。本轮生成模式、执行前确认偏好和已上传素材作为服务端校验的 turn policy 注入 Agent 上下文；模型结合完整对话决定正常回答、追问或调用一次生成工具。指定 Route 和 Composer 素材由 application 层强制绑定，模型参数不能替换它们。生成工具的 thinking、tool call、增量状态、tool result 和最终回复继续走同一 Agent SSE 与消息树，因此生成对话与普通工具对话具有一致的执行过程。

Chat 的“执行前确认”是单轮执行策略，不是 Session 模式。Agent 选择 Route 后，application 可先创建 `awaiting_confirmation` Run；该状态没有 Provider Job，Worker 不会领取。application 按“字段默认值、Route 默认值、明确输入”的优先级解析并持久化完整参数。用户在助手消息正文中的 Chat 参数卡确认后，application 按 Route Schema 重新校验最终 Prompt 与参数，并由 repository 在同一 SQLite 事务中把 Run 切换为 `queued`、创建首个 Job。Chat 以本地 `runId` 查询持久化 Run，将最新状态投影到执行步骤和消息活动卡，完成后在原消息中展示产物；原始 Pi tool result 只提供稳定关联和初始 review 数据。参数卡展示 Route Schema 的全部参数，不定义或消费 `advanced` 展示字段。用户修改值不会重新交给模型转述。

正常生成期间由 `generate_image` 或 `generate_video` 在单次工具执行内等待 SQLite Run，Worker 独立轮询供应商。Pi `tool_execution_update` 通过既有 Agent SSE 转发到 Chat，前端按稳定 `toolCallId` 原地更新标准化阶段；模型不负责定时轮询。执行前确认也是同一次工具执行中的暂停状态：工具先通过增量结果呈现确认卡，用户确认后继续等待原 Run，完成后把最终 Tool Result 交还模型生成本轮最终回复。`get_generation` 仅用于用户明确查询或中断恢复。工具结果使用本地 `runId` 标识 Po Agent 的持久化 Run，并把最新 Provider Job 的 `providerId` 与 `remoteTaskId` 标准化为 `providerTaskId` 返回；Chat 分别标注本地 Run ID 与供应商 Task ID。下载产物由 application 根据最终 Prompt 生成简短名称提示，filesystem adapter 负责过滤非法字符和 Windows 保留名，文件仍隔离在对应 Run 目录。
