...
千问返回了 HTTP 429。Worker 会遵守 `Retry-After` 并自动退避，不要手动重复创建任务。若频繁发生，应减少同时启用的批量工作流；临时上传 Policy 还受账号和模型维度的 100 QPS 限制，不适合作为高并发生产素材存储。

### `GENERATION_SUBMISSION_UNKNOWN`

请求可能已经到达供应商，但 Po Agent 没有收到确定响应。系统故意停止自动重提。先在千问平台确认是否已经创建任务，再由用户决定是否显式重试。
...
确认输入文件符合 Route 的类型和大小限制。下载只允许 HTTPS 阿里云 OSS 服务域名并禁止重定向；签名参数只用于即时下载，审计快照中会被脱敏。
