...
```curl
curl --location --request POST 'https://www.runninghub.cn/openapi/v2/rhart-video/sparkvideo-2.0-mini/multimodal-video' \
...
### 接口描述
```
name ： seedance2.0-Mini 
desc： seedance 2.0 Mini 多模态视频，支持文本、图片、视频、音频等多模态参考输入，面向批量创作、低成本视频编辑与参考生成。原生仅支持 480p/720p 生成；1080p/2k/4k 会先以 720p 生成，再通过超分补帧输出。适合高频生产，不适合追求 Standard 级画质稳定性的场景。
tags：高性价比批量生产|多模态参考|视频编辑续写|原生480p/720p|720p超分至1080p/2k/4k
```
...
| `resolution` | String | 必填 | 视频分辨率。Mini 原生提交 480p/720p；1080p/2k/4k 会先以 720p 生成，再进行超分补帧。<br>枚举值: [480p, 720p, 1080p, 2k, 4k] |
