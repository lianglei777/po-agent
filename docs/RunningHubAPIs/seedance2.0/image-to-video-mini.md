...
### 接口描述
```
name ： seedance2.0-Mini/图生视频
desc：seedance 2.0 Mini 图生视频，适合低成本批量将静态图片转为动态视频。原生仅支持 480p/720p 生成；1080p/2k/4k 会先以 720p 生成，再通过超分补帧输出。支持首帧/首尾帧模式、4-15 秒有声视频，面向高频内容生产而非电影级画质。
tags：高性价比批量生产|首帧/尾帧模式|原生480p/720p|720p超分至1080p/2k/4k|有声视频
```
...
| `resolution` | String | 必填 | 视频分辨率。分为模型原生输出的分辨率（480p、720p、native1080p），与基于 720p 原生生成后进行超分放大的分辨率（1080p、2k、4k）。<br>枚举值: [480p, 720p, native1080p, native4k, 1080p, 2k, 4k] |
...
| `conversionSlots` | List(String) | 可选 | 真人素材资产化槽位，多选；all 表示首帧与尾帧都做资产化。<br>枚举值: [all, firstFrameUrl, lastFrameUrl] |
