...
```curl
curl --location --request POST 'https://www.runninghub.cn/openapi/v2/seedream-v5-pro/image-to-image' \
...
--data-raw '{
  ...
  "height": 1024,
  "imageUrls": [
    "https://www.runninghub.cn/view?filename=2e9ba46efba3e57b4d909996a89284963561e89adc24a6ff0b52ad7b4aa17c26.png&type=input&subfolder=&Rh-Comfy-Auth=eyJ1c2VySWQiOiI1Mjk4NzEwMjNiMDU3MzY5NjZkZDQ2NGJiNjk1Y2FmYiIsInNpZ25FeHBpcmUiOjE3ODQxMzAxNzMzMzUsInRzIjoxNzgzNTI1MzczMzM1LCJzaWduIjoiMzZhYmQ0MzJjNzg3ZDBmOTgxMTI5OGI1OWU0ZmEzYzYifQ==&Rh-Identify=529871023b05736966dd464bb695cafb&rand=0.07750175907753243"
  ],
  "resolution": "2k",
  "outputFormat": "jpeg"
}'
...
| 参数说明 | 类型 | 必填/可选 | 说明 |
| --- | --- | --- | --- |
| `prompt` | String | 必填 | 文本长度限制: 5 - 5000 |
| `width` | Int | 可选 | 输入范围值: 240 - 8192 |
| `height` | Int | 可选 | 输入范围值: 240 - 8192 |
| `imageUrls` | List(String) | 必填 | 最多支持 10 项图片，每张 10 MB |
| `resolution` | String | 可选 | 优先级高于 width × height，传递 resolution 则使用 resolution，不再使用 width × height。<br>枚举值: [1k, 2k] |
| `outputFormat` | String | 可选 | 输出图片格式，可选 png / jpeg<br>枚举值: [jpeg, png] |
...
| ├ `url` | String | 重要提醒：该链接有效期仅为 24 小时。任务生成结束后，请务必在此时间窗口内将图片文件下载或转存至您的服务器。逾期后链接将永久失效且无法恢复。 |
| ├ `nodeId` | String | 生成该结果的工作流节点 ID |
| ├ `outputType` | String | 文件扩展名 (如 png, jpeg, txt) |
...
| `results` | List | 生成结果列表，包含图片或文本等输出 |
| ├ `url` | String | 重要提醒：该链接有效期仅为 24 小时。任务生成结束后，请务必在此时间窗口内将图片文件下载或转存至您的服务器。逾期后链接将永久失效且无法恢复。 |
| ├ `nodeId` | String | 生成该结果的工作流节点 ID |
| ├ `outputType` | String | 文件扩展名 (如 png, jpeg, txt) |
| └ `text` | String | 如果输出是纯文本，内容将显示在此字段 |
