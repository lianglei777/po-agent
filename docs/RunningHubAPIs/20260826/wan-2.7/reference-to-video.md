...
```curl
curl --location --request POST 'https://www.runninghub.cn/openapi/v2/alibaba/wan-2.7/reference-to-video' \
...
| `videoUrls` | List(String) | 可选 | 最多支持 5 项视频，每个 100 MB |
| `imageUrls` | List(String) | 可选 | 最多支持 5 项图片，每张 20 MB |
| `audioUrl` | String | 可选 | 音频URL，用于指定参考素材（图像/视频）中主体角色的音乐<br>最多支持 1 项音频，每个 15 MB |
| `negativePrompt` | String | 可选 | 文本长度限制: 0 - 500 |
| `resolution` | String | 必填 | 枚举值: [720P, 1080P] |
| `duration` | String | 必填 | 枚举值: [2, 3, 4, 5, 6, 7, 8, 9, 10] |
| `aspectRatio` | String | 必填 | 枚举值: [16:9, 9:16, 1:1, 4:3, 3:4] |
