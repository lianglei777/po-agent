...
画布节点不直接展示原生 `<video controls>`，避免播放器占满节点后阻断拖拽。

- 节点主体只展示 poster/预览帧；
- 中央播放按钮打开视频预览 Dialog；
- Dialog 中使用完整播放器，支持播放、暂停、进度、音量和浏览器全屏；
- Dialog 不因点击遮罩或 Escape 自动关闭，遵循当前项目 Dialog 规则；
- 关闭 Dialog 时暂停播放并释放不需要的媒体加载；
- 不在节点 hover 时自动播放，避免大量视频节点同时解码。
...
- capability 改变后重新加载匹配 Route；
- 如果原 Route 不再适用，选择默认 Route，并明确提示模型已切换；
