...
- Settings are grouped into bordered sections. Simple settings place the label and description on the left and the control on the right; compound forms such as provider credentials use a stacked layout so status, paths, inputs, and actions retain the full content width.
